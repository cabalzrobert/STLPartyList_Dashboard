...
namespace Comm.Common.Extensions
{
    public static class ObjectExt
    {
            ...
            try
            {
                if (source != null)
                {
                    if (source is JsonElement)
                    {
                        ...
                        try { source = (source as JObject).ToObject<T>(); } catch { source = null; }
                    else if (source is JArray)
                        try { source = (source as JArray).ToObject<T>(); } catch { source = null; }
                    else if (source is T)
                        source = (T)source;
        ...
        */
        ...
        private static FieldInfo gInfo;
        public static bool Has(this object settings, string name) //IsPropertyExist
        {
            gInfo = settings.GetType().GetField(name, BindingFlags.Instance | BindingFlags.Public);
            mInfo = settings.GetType().GetMethod(name, BindingFlags.Instance | BindingFlags.Public);
